...
public class SplinePath {
    ...
    public boolean updateRobot() {
        ...
        boolean inRange = true; // to get it to run the loop once and check

        while (inRange) {
            inRange = false;
            // check the next nextPointsRange points for a catch
            for (int i = robot.getActivePointIndex() + 1; i < Math.min(robot.getActivePointIndex() + nextPointsRange + 1, aimablePoints.length); i++) {
    ...
    public void changePath(Points newPoints) {
        this.points = newPoints;

        robot.setActivePointIndex(0);
    ...
    public double getActivePointIndex() {
        return robot.getActivePointIndex();
    }
}
