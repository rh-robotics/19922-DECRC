...
public class Launcher implements SubsystemBase {
    ...
    public void teleopUpdate(Gamepad currentGamepad1, Gamepad previousGamepad1, Gamepad currentGamepad2, Gamepad previousGamepad2) {
        ...
        if (shooting) {
            ...
            if ((currentGamepad1.triangle && !previousGamepad1.triangle) || (currentGamepad2.cross && !previousGamepad2.cross)) {
                if (override) {
                    override = false;
                    telemetry.addLine("Override off");
                } else {
                    override = true;
                    telemetry.addLine("Override on");
                }
            }
            ...
            if (!override) {
                ...
                if (currentGamepad1.dpad_down || currentGamepad2.dpad_down) {
                    double newPos = angler.getPosition() - 0.01;
                    angler.setPosition(Math.max(0, Math.min(1, newPos)));
                }
                if (currentGamepad1.dpad_up || currentGamepad2.dpad_up) {
                    double newPos = angler.getPosition() + 0.01;
                    angler.setPosition(Math.max(0, Math.min(1, newPos)));
                }
                ticksPerSecond = convertRPM(motorSpeed);
