...
public class DriveTrain extends SubsystemBase {
    ...
    public void setModulesOnPath(DrivePath path) { // in ratio to each other, turning clockwise
        ...
        setSpeeds(initialPositions, finalPositions, 0.1); // TODO: make this dynamic
    }
    ...
    public void setModulesWithGamepad(double gamepadX, double gamepadY, double turn) { // in ratio to each other, turning clockwise
        ...
        // leftFront, rightFront, rightRear, leftRear
        // initial positions are all known, assuming 0, 0 and 0 rotation. x / y =+- halfWidth * sqrt(2)
        double[][] initialPositions = {{-halfWidth, halfWidth}, {halfWidth, halfWidth}, {-halfWidth, -halfWidth}, {halfWidth, -halfWidth}};
        double[][] finalPositions = wheelPositions(teleopPath(gamepadX, gamepadY, turn));

        setDirections(initialPositions, finalPositions);

        double speed = Math.sqrt(Math.pow(gamepadX, 2) + Math.pow(gamepadY, 2)) / Math.sqrt(2);
        if (speed == 0 && turn != 0) {
            speed = Math.abs(turn);
        }
        setSpeeds(initialPositions, finalPositions, speed);
    }
    ...
    private void setDirections(double[][] initialPositions, double[][] finalPositions) {
        modules[0].setDirection(Math.atan2(finalPositions[0][1] - initialPositions[0][1], finalPositions[0][0] - initialPositions[0][0]) * 180 / Math.PI);
        modules[1].setDirection(Math.atan2(finalPositions[1][1] - initialPositions[1][1], finalPositions[1][0] - initialPositions[1][0]) * 180 / Math.PI);
        modules[2].setDirection(Math.atan2(finalPositions[2][1] - initialPositions[2][1], finalPositions[2][0] - initialPositions[2][0]) * 180 / Math.PI);
        modules[3].setDirection(Math.atan2(finalPositions[3][1] - initialPositions[3][1], finalPositions[3][0] - initialPositions[3][0]) * 180 / Math.PI);
    }
    ...
    private double[] teleopPath(double gamepadX, double gamepadY, double turn) {
        ...
        // robot rotation
        double angle = turn * (2 * Math.PI / 360) * DriveConstants.ROTATION_SPEED_CONSTANT;
